...
GEOPOLITICAL_KEYWORDS: list[str] = [
    "diplomacy", "sanctions", "embargo", "treaty", "alliance", "NATO", "UN", "G7", "BRICS", "EU", "IMF",
    "sovereignty", "conflict", "tension", "escalation", "deescalation", "war", "invasion", "military",
    "defense", "airstrike", "missile", "drone", "cyberattack", "terrorism", "insurgency", "ceasefire",
    "truce", "peace", "instability", "coup", "regime", "election", "protest", "strike", "unrest",
    "demonstration", "reform", "negotiation", "veto", "summit", "rivalry", "dominance",
]

MACRO_KEYWORDS: list[str] = [
    "policy", "regulation", "legislation", "tariff", "import", "export", "restriction", "blockade",
    "crisis", "collapse", "recession", "depression", "inflation", "deflation", "stagflation", "growth",
    "slowdown", "GDP", "unemployment", "output", "consumption", "demand", "supply", "stimulus",
    "subsidy", "tax", "deficit", "budget", "treasury", "bond", "yield", "interest", "rate", "hike",
    "cut", "easing", "tightening", "QE", "QT", "FederalReserve", "ECB", "BOJ", "PBOC", "RBI", "dovish", "hawkish",
]

PHYSICAL_KEYWORDS: list[str] = [
    "oil", "crude", "gasoline", "diesel", "LNG", "gas", "power", "grid", "blackout", "renewable",
    "solar", "wind", "nuclear", "uranium", "coal", "gold", "silver", "copper", "aluminum", "nickel",
    "lithium", "steel", "iron", "wheat", "corn", "soybean", "rice", "sugar", "coffee", "cocoa",
    "livestock", "cattle", "poultry", "fertilizer", "harvest", "drought", "flood", "hurricane",
    "cyclone", "typhoon", "storm", "wildfire", "earthquake", "volcano", "climate", "ElNino", "LaNina",
    "emissions", "carbon", "pollution", "ESG",
]

CORPORATE_KEYWORDS: list[str] = [
    "earnings", "revenue", "profit", "loss", "margin", "guidance", "surprise", "beat", "miss",
    "valuation", "PE", "ratio", "dividend", "buyback", "issuance", "IPO", "merger", "acquisition",
    "takeover", "spin-off", "restructuring", "layoffs", "hiring", "expansion", "partnership",
    "monopoly", "antitrust", "lawsuit", "litigation", "settlement", "fine", "compliance", "audit",
    "fraud", "scandal", "corruption", "CEO", "CFO", "governance",
]

TECH_KEYWORDS: list[str] = [
    "AI", "robotics", "automation", "machinelearning", "semiconductor", "chip", "shortage", "fabrication",
    "industrial", "patent", "startup", "venture", "liquidity", "leverage", "mortgage", "housing",
    "realestate", "urbanization", "infrastructure", "supplychain", "procurement", "outsourcing",
    "localization", "digitization", "cybersecurity", "hacking", "breach", "ransomware", "malware",
    "phishing", "encryption", "firewall", "cloud", "datacenter", "server", "5G", "satellite", "space",
    "defenseindustry", "aviation", "pandemic", "virus", "vaccine", "healthcare", "pharma", "biotech", "trial",
]

MARKET_MECHANICS_KEYWORDS: list[str] = [
    "volatility", "VIX", "spike", "crash", "rally", "bull", "bear", "correction", "breakout", "trend",
    "reversal", "momentum", "resistance", "support", "liquiditycrunch", "selloff", "speculation",
    "arbitrage", "hedging", "derivatives", "options", "futures", "swaps", "forex", "USD", "EUR", "JPY",
    "GBP", "CNY", "INR", "CAD", "AUD", "devaluation", "peg", "float", "systemic", "contagion",
    "spillover", "shock", "catalyst", "blackswans", "tailrisk",
]
...
class OracleState:
    """
    The final output of the Dhatu Oracle — the global market state distilled into a single actionable framework for Agent B.
    """
...
class NewsHarvester:
    ...
    def __init__(self, bus: "SharedIntelligenceBus | None" = None) -> None:
        ...
        if self.finnhub_key:
            ...
            logger.warning(
                "NewsHarvester: Finnhub Key MISSING. General market news feed will be offline."
            )
    ...
    async def _distill_headline(self, headline: str) -> dict:
        ...
        BULL_WORDS = [
            "BEAT", "UPGRADE", "RAISE", "POSITIVE", "GROWTH", "WIN", "SURGE", "PROFIT", "BULLISH",
            "SUCCESS", "EXPAND", "BUY", "OUTPERFORM", "RECOVERY", "BOOM", "HIKE", "SHORT SQUEEZE",
        ]
        BEAR_WORDS = [
            "MISS", "DOWNGRADE", "LOWER", "NEGATIVE", "FALL", "LOSS", "DROP", "PLUNGE", "BEARISH",
            "FAILURE", "SHRINK", "SELL", "UNDERPERFORM", "CRASH", "SLUMP", "CUT",
        ]

        for w in BULL_WORDS:
            if w in headline.upper():
                sentiment += 0.25
        for w in BEAR_WORDS:
            if w in headline.upper():
                sentiment -= 0.25
                    ...
                    if self.finnhub_key:
                        ...
                        if resp.status_code == 200:
                            ...
                            for item in data[:10]:
                                ...
                                is_dot = any(
                                    k.upper() in full_text for k in GEOPOLITICAL_KEYWORDS + MACRO_KEYWORDS
                                )
...
class TVNewsScent:
    ...
    def _parse_messages(self, raw_data: str) -> list[dict]:
        """ Decode TradingView ~m~ framed messages. Formula: ~m~length~m~JSON """
        results = []
        try:
            ptr = 0
        ...
        while self._running:
            try:
                ...
                async with websockets.connect(
                    ...
                    await ws.send(
                        self._format_message({"m": "news_create_session", "p": [self.session_id]})
                    )
                    ...
                    await ws.send(
                        self._format_message(
                            {
                                "m": "news_feed",
                                "p": [self.session_id, "news_feed:1:1:1:1", "ALL:ALL"],
                            }
                        )
                    )
...
class DhatuOracle:
    """
    Multi-stage LLM pipeline that synthesizes 500+ global variables into a single Dhatu state and risk protocol for the execution engine.
    ...
    def __init__(
        ...
        self._flash_keywords = {
            "CRASH", "HALT", "WAR", "EXPLOSION", "LIQUIDATION", "PANIC", "FLASH", "CIRCUIT",
        }
        self._last_flash_time = 0.0 # Throttler

        self._forbidden_neural = [
            "IGNORE ALL", "SYSTEM:", "USER:", "PREVIOUS INSTRUCTIONS", "SETTING:",
        ]
    ...
    async def _on_macro_received(self, data: dict) -> None:
        ...
        if len(self._macro_buffer) > 20:
            self._macro_buffer.pop(0)
    ...
    def calculate_spread_tension(self, bid: float, ask: float, volume: float) -> float:
        ...
        """
        spread = abs(ask - bid)
        if spread <= 0: # Inverted spread or zero spread = MAXIMUM TENSION
            return 100.0
    ...
    def evaluate_proposal(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if modifier < 0.40:
            ...
            reason = (
                f"CAUTION: High Risk Scaling ({modifier:.2f}x). Strikes permitted at reduced size."
            )
            ...
            try:
                ...
                try:
                    await asyncio.wait_for(
                        self._flash_event.wait(), timeout=self._refresh_interval_minutes * 60
                    )
    ...
    async def _fetch_yf_ticker_data(self, symbol: str, name: str) -> list[str]:
        ...
        try:
            ...
            df = await loop.run_in_executor(
                None, lambda t=ticker: t.history(period="5d", interval="1d"), # type: ignore
            )
            ...
            if not df.empty and len(df) >= 2:
                ...
                if symbol == "^TNX":
                    macro_tag = (
                        "YIELD_UP" if pct_change > 0.5 else ("YIELD_DOWN" if pct_change < -0.5 else "")
                    )
                elif symbol == "QQQ":
                    macro_tag = (
                        "NASDAQ_UP" if pct_change > 0.5 else ("NASDAQ_DOWN" if pct_change < -0.5 else "")
                    )
    ...
    async def fetch_macro_snapshot(self) -> dict[str, Any] | None:
        ...
        if not signals:
            logger.warning(
                "DhatuOracle: EMPTY macro snapshot detected. Returning None to trigger safety fallback."
            )
            return None
    ...
    async def _ingest_macro(self) -> list[str]:
        """ Ingest macroeconomic signals. Production: FRED / Yield Curve. Currently leveraging 10Y Note, DXY. """
        logger.debug("DhatuOracle: Ingesting macro data...")
        results = []
        r1 = await self._ticker_cache.get_or_fetch(
            "macro_TNX", lambda: self._fetch_yf_ticker_data("^TNX", "10-Year Treasury Yield"), ttl=300,
        )
    ...
    async def _ingest_physical(self) -> list[str]:
        """ Ingest physical world signals. Production: NOA[REDACTED] / EIA. Currently leveraging Crude Oil, Gold, Wheat. """
    ...
    async def _ingest_corporate(self) -> list[str]:
        """ Ingest corporate signals with breadth expansion. Check SPY (Cap-Weighted), RSP (Equal-Weighted), and IWM (Small-Cap). """
    ...
    async def _ingest_tech(self) -> list[str]:
        """ Ingest technology/cyber signals with sector expansion. Includes QQQ (Growth), SMH (Chips), and IGV (Software). """
    ...
    async def _ingest_market_mechanics(self) -> list[str]:
        """ Ingest market mechanics signals. Production: SpotGamma. Currently leveraging VIX and VVIX. """
    ...
    async def _rule_based_synthesis(self, signals: list[str]) -> OracleState:
        """ Deterministic fallback synthesis. Implements Absolute Value Parsing to prevent Semantic Drift. """
        ...
        for s in signals:
            ...
            if "VIX" in s_up:
                if val and val > 28.0:
                    ...
                    vix_abs = val
                if "+" in s or "UP" in s_up:
                    score -= 1 # Momentum

            # S&P 500 SENSING
            if "S&P 500" in s_up or "SPY" in s_up:
                if "-" in s or "DOWN" in s_up:
                    score -= 2
                if "+" in s or "UP" in s_up:
                    score += 1

            # YIELD / GOLD SENSING (CROSS-ASSET)
            if "YIELD" in s_up and val and val < 3.50:
                score -= 1 # Bond bid (Flight to quality)
            if "GOLD" in s_up and val and val > 2200:
                score -= 1 # Safe haven premium
        ...
        return OracleState(
            ...
            causation_summary=(f"Rule-Based Fallback: Score {score}. VIX Absolute: {vix_abs:.2f}."),
            confidence=0.35,
    ...
    async def _build_causation_graph(
        ...
        """ Multi-tier Graph Synthesis: Tier 1: Heuristic Engine, Tier 2: Global Resonance """
        return await self._synthesize_oracle_state(all_signals)

    def _evaluate_effort_needs(self, vix: float, uncertainty: float) -> str:
        """ Claude-Code Pattern: EFFORT_LEVELS ['low', 'medium', 'high', 'max'] Scales thinking budget based on market disorder. """
    ...
    async def _synthesize_oracle_state(self, signals: list[str]) -> CausationGraph:
        """ Synthesizes the global macro regime using Adaptive Effort Scaling logic ported from Anthropic's Claude-Code codebase. """
    ...
    async def _map_to_dhatu_state(
        ...
        """ Semantic Archetype Resonance. Actual reasoning without Ollama via Vector Similarity. """
        logger.info("DhatuOracle: Initiating Semantic Resonance Chain...")

        # 1. Prepare Narrative Vector
        narrative = (
            f"Theme: {graph.dominant_theme}. Bias: {graph.macro_bias}. Certainty: {graph.certainty:.1%}. "
            + ". ".join(all_signals)
        )
    ...
    async def _full_synthesis_cycle(self) -> OracleState | None:
        ...
        logger.info(
            f"DhatuOracle: {len(all_signals)} total signals (including pipeline intel) collected for synthesis."
        )
