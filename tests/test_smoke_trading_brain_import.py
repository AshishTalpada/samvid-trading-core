...
def test_apply_runtime_safety_allows_live_when_authorized(tmp_path):
    """Startup safety preserves live only with force-live plus promotion approval."""
    src_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "src"))
    if src_path not in sys.path:
        sys.path.insert(0, src_path)
    ...
    with patch.dict(
        os.environ,
        {
            "ALLOW_FORCE_LIVE": "1",
            "TRADING_MODE": "live",
            "SOVEREIGN_PROMOTION_READINESS_REPORT": str(report),
        },
        clear=False,
    ):
        with patch("safety.send_telegram_alert", return_value=None):
