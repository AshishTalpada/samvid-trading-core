...
@pytest.mark.asyncio
async def test_recovery_treats_now_timestamp_as_recent_for_active_orders(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    data_dir = tmp_path / "data"
    ...
    db.execute(
        ...
        "(orderId, symbol, status, filled, remaining, last_update, parent_id, intent_id) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        (126, "WMT", "PreSubmitted", 0, 2, "now", 0, None),
    )
    db.commit()
    db.close()
    ...
    db = sqlite3.connect(data_dir / "trading.db")
    status = db.execute(
        "SELECT status FROM persistent_orders WHERE orderId = 126"
    ).fetchone()[0]
    assert status == "PreSubmitted"
    assert conn._recovered_orders == set()
