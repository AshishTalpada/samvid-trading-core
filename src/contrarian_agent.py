...
class ContrarianAgent:
    ...
    def evaluate_crowd_error(
        self,
        retail_long_ratio: float,
        retail_short_ratio: float,
        influencer_bull_mentions: int,
        influencer_bear_mentions: int,
    ) -> dict[str, Any]:
        """
        ...
        if total_influencer == 0:
            ...
            influencer_imbalance = (
                influencer_bull_mentions - influencer_bear_mentions
            ) / total_influencer
        ...
        if self.crowd_sentiment_score > 0.75:
            ...
            logger.warning(
                "Contrarian Agent: Extreme crowd euphoria detected. Generating SELL signal."
            )
