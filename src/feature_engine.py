...
class RecursiveFeatureEliminator:
    ...
    def eliminate(
        self, feature_names: List[str], importances: np.ndarray, threshold: float = 0.0
    ) -> List[str]:
        kept = [
            name for name, imp in zip(feature_names, importances, strict=False) if imp > threshold
        ]
        removed = [
            name for name, imp in zip(feature_names, importances, strict=False) if imp <= threshold
        ]
        if removed:
            logger.info(f"[FEATURE ENGINE] Pruned {len(removed)} features: {removed[:5]}...")
        return kept
