...
class KolmogorovVetter:
    ...
    def vet(self, reasoning: str, max_complexity: float = 0.6) -> dict[str, Any]:
        ...
        contradiction = bool(re.search(r"\b(but|however|although|yet)\b.*\b(but|however|although|yet)\b", reasoning, re.IGNORECASE))
        trusted = complexity <= max_complexity and not contradiction and word_count < 200
        if not trusted:
            logger.warning(f"[VETTER] Reasoning rejected. complexity={complexity:.2f}, contradiction={contradiction}, words={word_count}")
        return {"trusted": trusted, "complexity": round(complexity, 3), "contradiction": contradiction, "word_count": word_count}
