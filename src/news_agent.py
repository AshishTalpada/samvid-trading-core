...
FED_KEYWORDS = {
    "hawkish": ["raise rates","rate hike","tighten","restrictive","reduce balance","quantitative tightening","not cutting"],
    "dovish":  ["cut rates","lower rates","pause","accommodative","easing","support growth","quantitative easing"],
}
...
class MacroNewsAgent:
    ...
    def classify_fed_statement(self, text: str) -> Dict:
        ...
        return {"stance": stance, "score": round(score, 3), "hawkish_hits": hawk, "dovish_hits": dove}
    ...
    def rate_move_probability(self, statement_score: float, cpi_yoy: float, unemployment: float) -> float:
        hike_prob = 0.5 + statement_score * 0.3 + (cpi_yoy - 2.0) * 0.05 - (unemployment - 4.0) * 0.03
