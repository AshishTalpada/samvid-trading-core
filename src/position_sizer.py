...
class PositionSizer:
    ...
    @staticmethod
    def fixed_risk(equity: float, risk_pct: float, entry: float,
                   stop: float, min_shares: int = 1) -> float:
        ...
        if stop_distance < 0.001:
            logger.warning(f"PositionSizer: Near-zero stop distance ({stop_distance:.4f}). Min shares.")
            return float(min_shares)
    ...
    def kelly(win_rate: float, avg_win: float, avg_loss: float,
              fraction: float = 0.25, max_pct: float = 0.20) -> float:
    ...
    @staticmethod
    def validate(shares: float, price: float, equity: float,
                 max_notional_pct: float = 0.15,
                 max_notional_abs: float | None = None,
                 min_shares: int = 1) -> float:
        ...
        if result < shares:
            logger.debug(f"PositionSizer: Capped {shares:.2f}→{result:.2f} (limit ${result*price:,.0f})")
        return result
    ...
    @classmethod
    def size_trade(cls, equity: float, risk_pct: float, entry: float, stop: float,
                   win_rate: float = 0.5, avg_win: float = 0.0, avg_loss: float = 0.0,
                   max_notional_pct: float = 0.15, max_notional_abs: float | None = None,
        ...
        return cls.to_whole_shares(cls.validate(raw, entry, equity, max_notional_pct, max_notional_abs))
