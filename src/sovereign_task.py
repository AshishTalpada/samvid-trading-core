...
class SovereignTask:
    ...
    def __init__(self, task_id: str, task_type: str, description: str, metadata: dict = None):
        ...
        # ── AGENTIC DIAGNOSTICS (SETO V35.0) ──
    ...
    def log(self, message: str):
        # 1. Persistent File Log (SETO V12.1: Non-blocking optimization)
        try:
        ...
        # 2. Real-time Terminal Log (SETO V12.0 Visibility)
    ...
    def save(self):
        ...
        for attempt in range(5):
            try:
                ...
                # GAP-217 FIX: Robust atomic replace for Windows
...
class TaskManager:
    """Orchestrates the lifecycle of Sovereign Tasks (SETO V35.1 Hardened)."""
    def __init__(self, registry_path: str = "data/active_tasks.json"):
        self.registry_path = registry_path
        self.tasks: Dict[str, SovereignTask] = {}
        self._symbol_index: Dict[str, List[str]] = {} # Symbol -> List of Task IDs (GAP-244 FIX)
        os.makedirs(os.path.dirname(self.registry_path), exist_ok=True)
        self.load_registry()

    def spawn_trade(self, symbol: str, setup: dict) -> SovereignTask:
        # GAP-105 FIX: Proactive memory management
    ...
    def load_registry(self):
        """GAP-101 FIX: Reconstruct full task state from registry."""
    ...
    def save_registry(self):
        """GAP-103 FIX: Atomic save with full state preservation and Windows retry."""
        try:
    ...
    def purge_completed(self, max_age_days: int = 7):
        """GAP-104 FIX: Prevents memory leaks by clearing old finished and stale tasks."""
        now = time.time()
