...
class DynamicHedger:
    ...
    def compute_hedge_contracts(self, position_delta: float, option_delta: float, contract_multiplier: int = 100) -> int:
        if abs(option_delta) < 1e-6: return 0
    ...
    def hedge_signal(self, pnl_pct: float, threshold: float = -0.015) -> str:
        if pnl_pct < threshold:
            logger.warning(f"[HEDGER] PnL={pnl_pct:.2%} below threshold. Recommend protective hedge.")
            return "HEDGE_NOW"
        return "HOLD"
