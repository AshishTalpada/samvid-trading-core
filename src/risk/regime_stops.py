...
class RegimeAdaptiveStops:
    ...
    def compute_stops(self, entry: float, side: str, regime: str,
        ...
        logger.info(f"[REGIME STOPS] {regime} | Entry={entry:.2f} Stop={stop:.2f} Target={target:.2f} ATR={atr:.4f}")
        return {"stop_loss": round(stop, 4), "take_profit": round(target, 4), "atr": round(atr, 4), "regime": regime}
