...
class BlackSwanCrashSimulator:
    ...
    def generate_crash_path(
        self, days: int = 30, severity: str = "moderate", start_price: float = 100.0
    ) -> List[float]:
        severity_params = {
        ...
        logger.info(
            f"[CRASH SIM] {severity} scenario: {start_price:.0f} -> {prices[-1]:.0f} ({(prices[-1] / start_price - 1) * 100:.1f}%)"
        )
        return prices

    def run_stress_suite(self, start_price: float = 100.0) -> dict[str, List[float]]:
        return {
            sev: self.generate_crash_path(severity=sev, start_price=start_price)
