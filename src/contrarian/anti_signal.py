...
class AntiSignalTracker:
    ...
    def compute_retail_sentiment(
        self, put_call_ratio: float, aaii_bull_pct: float, retail_flow_usd: float
    ) -> float:
    ...
    def fade_signal(self, retail_sentiment: float) -> str:
        if retail_sentiment > self.threshold:
            logger.warning(
                f"[ANTI-SIGNAL] Extreme retail bullishness ({retail_sentiment:.0%}) → FADE LONG, go short."
            )
            return "FADE_LONG"
        if retail_sentiment < (1 - self.threshold):
            logger.warning(
                f"[ANTI-SIGNAL] Extreme retail bearishness ({retail_sentiment:.0%}) → FADE SHORT, go long."
            )
            return "FADE_SHORT"
        return "NEUTRAL"
