...
class MindMath:
    ...
    async def _tool_validate_geometry(
        ...
        # SETO V22.4: Set precision for institutional math (9 places for micro-cap assets)
        getcontext().prec = 28
        ...
        try:
            ...
            if direction.upper() == "LONG":
                if d_stop >= d_entry:
                    return {"valid": False, "reason": "Deterministic VETO: Stop loss must be BELOW entry for Longs."}
                if d_target <= d_entry:
                    return {"valid": False, "reason": "Deterministic VETO: Target must be ABOVE entry for Longs."}
            elif direction.upper() == "SHORT":
                if d_stop <= d_entry:
                    return {"valid": False, "reason": "Deterministic VETO: Stop loss must be ABOVE entry for Shorts."}
                if d_target >= d_entry:
                    return {"valid": False, "reason": "Deterministic VETO: Target must be BELOW entry for Shorts."}
            ...
            if d_atr is not None:
                ...
                if d_atr <= (d_entry * Decimal("1e-7")) or d_atr <= 0:
                    return {"valid": False, "reason": "Deterministic VETO: Zero/Near-Zero ATR detected. Market is stagnant or feed is dead."}
                ...
                if risk < (d_atr * Decimal("0.5")):
                    return {"valid": False, "reason": f"Deterministic VETO: Stop distance ({float(risk):.4f}) is < 0.5 ATR ({float(d_atr)*0.5:.4f}). High noise risk."}
