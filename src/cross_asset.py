...
class CrossAssetLeadIndicator:
    ...
    def yield_curve_slope(self, y10: float, y2: float) -> float:
        slope = y10 - y2
        if slope < 0: logger.warning(f"[CROSS ASSET] Inverted yield curve: slope={slope:.2f}bps")
        return slope
    ...
    def leading_signal(self, asset_returns: Dict[str, float], y10: float, y2: float) -> str:
        ...
        if appetite > 0.3 and slope > 0: return "RISK_ON"
        if appetite < -0.3 or slope < 0: return "RISK_OFF"
        return "NEUTRAL"
