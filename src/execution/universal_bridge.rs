...
impl FixBridge {
    ...
    pub async fn send_order(&mut self, symbol: &str, side: &str, qty: f64, price: f64) -> Result<(), Box<dyn Error>> {
        // 11=ClOrdID, 21=HandlInst(1=Auto), 55=Symbol, 54=Side(1=Buy,2=Sell), 60=TransactTime, 38=Qty, 40=OrdType(2=Limit), 44=Price
        let cl_ord_id = format!("SOV-{}", SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_micros());
        let body = format!(
            "11={}\x0121=1\x0155={}\x0154={}\x0160={}\x0138={}\x0140=2\x0144={}\x01",
            cl_ord_id, symbol, side, SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis(), qty, price
        );
        
        let msg = self.construct_message("D", &body);
        println!("[FIX_BRIDGE] Submitting Order: {}", msg.replace("\x01", "|"));
