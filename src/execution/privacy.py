...
class DifferentialPrivacyObfuscator:
    ...
    def generate_twap_slices(self, total_quantity: int, num_slices: int) -> list[int]:
        ...
        active_slices = min(num_slices, total_quantity)
        base_slice = total_quantity / active_slices
        slices = []
        ...
        for i in range(active_slices - 1):
            ...
            actual_slice = min(actual_slice, total_quantity - sum(slices) - remaining_after_this)
        ...
        if final_slice <= 0 and slices:
            logger.warning("DP Obfuscator produced non-positive final slice; rebalancing.")
            slices[-1] = max(1, slices[-1] + final_slice - 1)
            final_slice = total_quantity - sum(slices)

        slices.append(max(1, final_slice))
        return slices
