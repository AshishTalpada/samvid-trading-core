...
impl Default for FastDecisionEngine {
    fn default() -> Self {
        Self::new()
    }
}
...
impl FastDecisionEngine {
    ...
    pub fn new() -> Self {
        FastDecisionEngine {
            required_agents: vec![
                ...
                "Agent_F".to_string(),
                "Agent_G".to_string(),
                "Risk_Guard".to_string(),
                "Dhatu_Oracle".to_string(),
                "Swarm_Predictor".to_string(),
                "Mind_Ultrathink".to_string(),
            ],
        }
    }
    ...
    pub fn evaluate<'py>(
        ...
        py: Python<'py>,
        context: &'py PyDict,
        agent_outputs: &'py PyList,
    ) -> PyResult<&'py PyDict> {
        let is_probe: bool = context
            .get_item("is_probe")?
            .map(|v| v.extract().unwrap_or(false))
            .unwrap_or(false);
        let regime: String = context
            .get_item("regime")?
            .map(|v| v.extract().unwrap_or("UNKNOWN".to_string()))
            .unwrap_or("UNKNOWN".to_string());
        let safe_mode_active: bool = context
            .get_item("safe_mode_active")?
            .map(|v| v.extract().unwrap_or(false))
            .unwrap_or(false);
        ...
        if num_agents < self.required_agents.len() && !is_probe {
            return self.reject(
                py,
                &format!(
                    "Quorum Violation: Expected at least {}, got {}.",
                    self.required_agents.len(),
                    num_agents
                ),
            );
        }
        ...
        for i in 0..num_agents {
            ...
            let agent: String = out
                .get_item("agent")?
                .map(|v| v.extract().unwrap_or("UNKNOWN".to_string()))
                .unwrap_or("UNKNOWN".to_string());
            let vote: String = out
                .get_item("vote")?
                .map(|v| v.extract().unwrap_or("ABSTAIN".to_string()))
                .unwrap_or("ABSTAIN".to_string());
            let confidence: f64 = out
                .get_item("confidence")?
                .map(|v| v.extract().unwrap_or(0.0))
                .unwrap_or(0.0);
            let agent_ts: f64 = out
                .get_item("timestamp")?
                .map(|v| v.extract().unwrap_or(0.0))
                .unwrap_or(0.0);
            ...
            if agent == "Risk_Guard" && vote == "NO" && !is_probe {
                return self.reject(
                    py,
                    "🛑 HARD VETO: Risk_Guard blocked the trade (Safety Protocol).",
                );
            }
        ...
        let avg_confidence = if active_voters > 0 {
            total_confidence / active_voters as f64
        } else {
            0.0
        };
        ...
        if safe_mode_active {
            ...
            if actual_threshold >= 0.50 && (avg_confidence > 0.70 || is_probe) {
                self.execute(
                    ...
                    avg_confidence,
                    "🏛️ SAFE-MODE SUCCESS",
                    agent_d_edge_crowded,
                )
            } else {
                self.reject(py, "🛑 SAFE-MODE REJECTION: Consensus < 50%.")
            }
        } else {
            let min_required = if regime == "CHOPPY" || regime == "VOLATILE" {
                4.0
            } else {
                5.0
            };
            let vote_ratio = if active_voters > 0 {
                yes_votes / active_voters as f64
            } else {
                0.0
            };
            ...
            if yes_votes >= min_required && vote_ratio >= 0.60 && avg_confidence > 0.60 {
                self.execute(py, avg_confidence, "Quorum Met", agent_d_edge_crowded)
            } else {
                self.reject(py, "Consensus Failure")
            }
        }
    }
