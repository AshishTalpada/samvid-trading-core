...
class ESGAlphaAgent:
    ...
    def estimate_esg_alpha(self, esg_scores: Dict[str, float]) -> Dict:
        ...
        signal = (
            "OVERWEIGHT" if composite > 0.65 else "UNDERWEIGHT" if composite < 0.35 else "NEUTRAL"
        )
        logger.info(
            f"[ESG] Composite={composite:.2f} | Alpha={alpha_estimate_bps:+.1f}bps | Signal={signal}"
        )
        return {
            "composite": round(composite, 3),
            "alpha_bps": round(alpha_estimate_bps, 2),
            "signal": signal,
            "breakdown": {"E": e, "S": s, "G": g},
        }

    def governance_risk_flag(
        self, insider_ownership_pct: float, board_independence_pct: float
    ) -> bool:
        risky = insider_ownership_pct > 70 or board_independence_pct < 30
        if risky:
            logger.warning(
                f"[ESG] Governance risk: insider={insider_ownership_pct:.0f}% board_ind={board_independence_pct:.0f}%"
            )
        return risky
