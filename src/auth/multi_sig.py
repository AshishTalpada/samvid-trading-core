...
class MultiSigAuthorizer:
    ...
    def request_change(self, change_id: str, parameter: str, new_value: float) -> str:
        self._pending[change_id] = {"parameter": parameter, "value": new_value, "sigs": [], "ts": time.time()}
        logger.info(f"[MULTISIG] Change request {change_id}: {parameter}={new_value}. Awaiting {self.required} signatures.")
        return change_id
    ...
    def sign(self, change_id: str, signer_id: str, secret: str) -> bool:
        ...
        logger.info(f"[MULTISIG] {signer_id} signed {change_id}. {len(entry['sigs'])}/{self.required} collected.")
        return True
    ...
    def is_approved(self, change_id: str) -> bool:
        ...
        if approved:
            logger.info(f"[MULTISIG] Change {change_id} APPROVED. Applying {entry['parameter']}={entry['value']}")
        return approved
