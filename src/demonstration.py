...
async def run_demonstration():
    ...
    agents = ["Agent A (Patterns)", "Agent B (Sentiment)", "Agent C (Executor)", "Agent D (Learning)", "Dhatu Oracle"]
    for agent in agents:
    ...
    await asyncio.sleep(1)
    regimes = ["VRIDDHI (Expansion)", "STHITI (Stationary)", "KSHAYA (Contraction)", "ABHAVA (Void)"]
    ...
    for sym in symbols:
        ...
        print(f"    {Colors.CYAN}🎯 DISCOVERY:{Colors.END} {sym} matched {Colors.BOLD}BULL_FLAG{Colors.END} ({confidence:.1f}%)")
    ...
    for agent, vote, conf, reason in votes:
        await asyncio.sleep(0.5)
        print(f"    {Colors.BOLD}{agent}:{Colors.END} [{Colors.GREEN}{vote}{Colors.END}] | Confidence: {conf:.2f} | {Colors.BLUE}{reason}{Colors.END}")
    ...
    print(f"    Exit Intelligence: {Colors.YELLOW}ACTIVE (Monitoring Posterior Beliefs...){Colors.END}")
