...
class MarketReflexModel:
    ...
    def nash_optimal_size(
        self, intended_size: float, bid_ask_spread: float, max_tolerable_impact: float
    ) -> float:
        ...
        logger.info(
            f"[REFLEX] Capped size at {optimal:.0f} to prevent triggering HFT front-running."
        )
        return optimal
