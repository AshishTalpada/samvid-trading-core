...
class CEOVoiceDeceptionAgent:
    ...
    def analyse_prosody(self, f0_values: list[float], amplitude_values: list[float]) -> Dict:
        ...
        shimmer = sum(abs(amplitude_values[i] - amplitude_values[i-1]) for i in range(1, len(amplitude_values)))
        ...
        logger.info(f"[VOICE] Jitter={jitter:.3f} Shimmer={shimmer:.3f} Stress={stress:.2f} -> {verdict}")
        return {"stress_score": round(stress, 3), "jitter": round(jitter, 4), "shimmer": round(shimmer, 4), "verdict": verdict}
