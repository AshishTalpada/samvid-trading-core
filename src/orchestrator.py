    ...
    def assign_task(self, task_name: str, required_vram_gb: float) -> Optional[str]:
        ...
        for device, free in sorted(self.device_vram.items(), key=lambda x: -x[1]):
            if free >= required_vram_gb:
                ...
                logger.debug(f"[ORCHESTRATOR] Assigned '{task_name}' to {device} ({free:.1f}GB free)")
                return device

        logger.warning(f"[ORCHESTRATOR] OOM WARNING: No device has {required_vram_gb}GB VRAM for '{task_name}'")
        return None
