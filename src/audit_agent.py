...
class AuditAgent:
    ...
    def full_audit(self, agent_outputs: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if len(confidences) >= 3:
            ...
            if conf_std < 0.02:
                issues.append(f"ANCHORING: All agents report near-identical confidence (std={conf_std:.4f}). Likely copying each other.")
                risk_score += 0.25
        ...
        if flip_count > len(agent_outputs) * 0.5:
            issues.append(f"MASS_FLIP: {flip_count}/{len(agent_outputs)} agents reversed their vote since last cycle. Potential regime instability.")
            risk_score += 0.35
        ...
        if confidences:
            ...
            if spread > 0.6:
                issues.append(f"CONFIDENCE_DIVERGENCE: Spread={spread:.2f}. Agents have wildly different certainty. Market ambiguity high.")
                risk_score += 0.15

        passed = risk_score < 0.5
        if not passed:
            logger.warning(f"[AUDIT] Cycle {self._cycle_count} failed. RiskScore={risk_score:.2f}. Issues: {issues}")
        else:
