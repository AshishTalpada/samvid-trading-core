...
# ---------------------------------------------------------------------------
# Safe OpenBB import — DEFERRED until initialization (SETO V22.6)
...
class OpenBBProvider:
    ...
    async def _ensure_obb(self) -> bool:
        """
        SETO V22.6: Non-blocking lazy loader for the OpenBB SDK.
        ...
        async with self._load_lock:
            ...
            try:
                ...
                _OPENBB_AVAILABLE = False
                return False
        # One-time Credential Sync after first successful load (SETO V22.8)
        try:
            from vault import Vault
            pat = Vault.get("OPENBB_PAT")
            
            # SETO V22.6: Activate PAT if available (Prioritize Hub Sync)
    ...
    async def fetch_technical_indicators(
        ...
        try:
            ...
            if period_days > 100:
                logger.warning(f"RSI/Technical period {period_days} too high, capping at 100 to prevent lag (SETO V22.5).")
                period_days = 100
