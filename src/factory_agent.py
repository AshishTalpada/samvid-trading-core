...
class FactoryActivityDetector:
    ...
    """
    def analyse_thermal_signature(self, thermal_grid: np.ndarray, baseline_grid: np.ndarray) -> Dict:
        ...
        logger.info(f"[FACTORY] Mean delta: {mean_delta:.2f}°C | Hot: {hot_pixels} | Cold: {cold_pixels} | Status: {status}")
        return {"status": status, "mean_delta_celsius": round(mean_delta, 2), "hot_pixels": hot_pixels}
