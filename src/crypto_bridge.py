...
class CryptoBridgeAgent:
    ...
    def get_crypto_returns(self, coins: list[str] | None = None) -> Dict[str, float]:
        ...
        try:
            r = requests.get(self.COINGECKO_API, params={"ids": ",".join(coins), "vs_currencies": "usd", "include_24hr_change": "true"}, timeout=5)
            data = r.json()
    ...
    def risk_signal(self, btc_24h_return: float) -> str:
        if btc_24h_return < -0.05: return "RISK_OFF"
        if btc_24h_return > 0.05: return "RISK_ON"
        return "NEUTRAL"
