...
class MarketGANTrainer:
    ...
    def train_step(self, real_data: np.ndarray) -> None:
        ...
        if len(self.d_loss_history) % 100 == 0:
            logger.info(
                f"[GAN TRAINER] Step {len(self.d_loss_history)} | D_Loss: {d_loss:.3f} | G_Loss: {g_loss:.3f}"
            )
    ...
    def generate_synthetic_crash(self) -> np.ndarray:
        ...
        logger.warning(
            f"[GAN TRAINER] Synthesised Black Swan sequence (Drop: {((100 - synthetic_path[-1]) / 100):.1%})"
        )
